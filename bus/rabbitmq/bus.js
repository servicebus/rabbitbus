...
function RabbitMQBus (options, implOpts) {
  ...
  this.assertQueuesOnFirstSend =
    options.assertQueuesOnFirstSend === undefined
      ? true
      : options.assertQueuesOnFirstSend
  this.channels = []
...
RabbitMQBus.prototype.unlisten = function unlisten (queueName, options) {
  if (this.queues[queueName] === undefined) {
    throw new Error(
      util.format('no queue currently listening at %s', queueName)
    )
  } else {
...
RabbitMQBus.prototype.destroyListener = function removeListener (
  queueName,
  options
) {
  options = options || {}
  if (!options.force && this.queues[queueName] === undefined) {
    ...
    if (!q && options.force) {
      ...
      this.listenChannel
        .deleteQueue(queueName, { ifEmpty: false })
        .then(function (ok) {
          em.emit('success')
        })
      return em
...
RabbitMQBus.prototype.send = function send (queueName, message, options, cb) {
  ...
  if (cb && !this.confirmChannel)
    return cb(
      new Error(
        'callbacks only supported when created with bus({ enableConfirms:true })'
      )
    )
  ...
  var key =
    this.confirmChannel && cb
      ? options.queueName + '.confirm'
      : options.queueName
  ...
  this.handleOutgoing(
    ...
    message,
    options,
    function (queueName, message, options) {
...
RabbitMQBus.prototype.subscribe = function subscribe (
  queueName,
  options,
  callback
) {
...
RabbitMQBus.prototype.publish = function publish (
  queueName,
  message,
  options,
  cb
