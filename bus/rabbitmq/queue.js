...
function Queue (options) {
  ...
  extend(queueOptions, {
    autoDelete:
      options.autoDelete === undefined
        ? !(options.ack || options.acknowledge)
        : options.autoDelete,
    contentType: options.contentType || 'application/json',
    durable: Boolean(options.ack || options.acknowledge),
    exclusive: options.exclusive || false,
    persistent: Boolean(
      options.ack || options.acknowledge || options.persistent
    )
  })

  this.ack = options.ack || options.acknowledge
  this.assertQueue =
    options.assertQueue === undefined ? true : options.assertQueue
  ...
  if (!this.assertQueue) {
    ...
    this.listenChannel
      .assertQueue(this.queueName, this.queueOptions)
      .then(function (_qok) {
        if (self.ack) {
          ...
          self.listenChannel
            .assertQueue(self.errorQueueName, self.queueOptions)
            .then(function (_qok) {
              self.initialized = true
              self.emit('ready')
            })
  ...
  this.listenChannel
    .consume(
      ...
      */
        ...
        message.content = options.formatter.deserialize(
          message.content.toString()
        )
        options.queueType = 'queue'
        self.bus.handleIncoming(self.listenChannel, message, options, function (
          channel,
          message,
          options
        ) {
...
Queue.prototype.destroy = function destroy (options) {
  ...
  this.listenChannel.deleteQueue(this.queueName).then(function (ok) {
    em.emit('success')
  })
...
Queue.prototype.unlisten = function unlisten () {
  ...
  if (this.listening) {
    this.listenChannel
      .cancel(this.subscription.consumerTag)
...
Queue.prototype.send = function send (event, options, cb) {
  ...
  options.persistent = Boolean(
    options.ack || options.acknowledge || options.persistent || self.ack
  )
  ...
  channel.sendToQueue(
    ...
    new Buffer(options.formatter.serialize(event)),
    options,
    cb
  )
}
