...
function PubSubQueue (options) {
  ...
  this.exchangeName =
    options.exchangeName || this.bus.exchangeName || 'amq.topic'
  ...
  this.sendChannel.assertExchange(
    this.exchangeName,
  ...
  if (this.confirmChannel) {
    this.confirmChannel.assertExchange(
      this.exchangeName,
      this.exchangeOptions.type || 'topic',
      this.exchangeOptions
    )
...
PubSubQueue.prototype.subscribe = function subscribe (options, callback) {
  ...
  this.log(
    'subscribing to queue %j with routingKey %j',
    this.queueName,
    this.routingKey
  )
  ...
  function _unsubscribe (cb) {
    if (subscribed) {
      ...
      self.listenChannel.cancel(subscription.consumerTag).then(function () {
    ...
    self.listenChannel
      .consume(
        ...
        */
          ...
          self.bus.handleIncoming(
            self.listenChannel,
            message,
            options,
            function (channel, message, options) {
  ...
  self.correlator.queueName(options, function (err, uniqueName) {
    if (err) throw err
    self.listenChannel
      .assertQueue(uniqueName, self.queueOptions)
      .then(function (qok) {
        return self.listenChannel.bindQueue(
          uniqueName,
          self.exchangeName,
          self.routingKey || self.queueName
        )
      ...
      .then(function () {
        if (self.ack) {
          ...
          self.listenChannel
            .assertQueue(self.errorQueueName, errorQueueOptions)
            .then(function (_qok) {
