...
describe('messageDomain', function () {
  ...
  xit('should send to error queue after five retries', function (done) {
    ...
    domainBus.listen('my.message.domain.4.error', { ack: true }, function (
      event
    ) {
      count.should.equal(6) // one send and five retries
      event.handle.ack()
      domainBus
        .destroyListener('my.message.domain.4')
        .on('success', function () {
          domainBus
            .destroyListener('my.message.domain.4.error')
            .on('success', function () {
              done()
            })
        })
