  ...
  describe('#subscribe & #unsubscribe', function () {
    describe('on temporary queues', function () {
      it('should complete subscribe/unsubscribe cycle', function (done) {
        const subscription = bus.subscribe('test.subunsub.1', function (
          event
        ) {})

        subscription.should.be.a.Object().and.have.ownProperty('unsubscribe')

        subscription.unsubscribe.should.be.a.Function()
      ...
      it('should not receive events after multiple subscribes/unsubscribes', function (done) {
        ...
        const subscription1 = bus.subscribe('test.subunsub.3', function (
          event
        ) {
          receivedEvents += 1
        })
        const subscription2 = bus.subscribe('test.subunsub.3', function (
          event
        ) {
          receivedEvents += 1
        })
